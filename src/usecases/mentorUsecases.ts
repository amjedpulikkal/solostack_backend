...
} from "../infrastructure/services/maileTamplate";
...
import {
  IreviewTimeRepository,
  IreviewRepository,
} from "@interfaces/repositroey/IreviewRepository";
...
export class MentorUseCases implements ImentorUseCases {
  ...
  async verifyMentorAccount(email: string, otp: string): Promise<ResponseObj> {
    ...
    const otpValid = (await this.otpRepository.verifyOtp(
      email,
      otp
    )) as singUpBody;
    console.log("Otp-------------->", otpValid);
    if (!otpValid) return { status: 403, data: "otp not valid" };
  ...
  async login(email: string, password: string): Promise<ResponseObj> {
    try {
      ...
      const isPasswordCorrect = await this.hashPassword.comparePassword(
        password,
        user.password
      );
  ...
  async updateAvailableTime(
    mentor: Imentor,
    date: { date: Date; time: number[] }
  ): Promise<ResponseObj> {
  
    const data = await this.reviewTimeRepository.createNewReview(
      date.date,
      date.time,
      mentor._id
    );
  ...
  async getAvailableTime(mentor: Imentor, date: Date): Promise<ResponseObj> {
    ...
    const data = await this.reviewTimeRepository.getAvailableTime(
      mentor._id,
      date
    );
  ...
  async getAllMentorAvailableTime(
    ...
    const data = await this.reviewTimeRepository.getAllMentorAvailableTime(
      date,
      time
    );
  ...
  async getAllMentors(date: Date): Promise<ResponseObj> {
    if (date) {
      const allMentors = await this.reviewTimeRepository.getAllMentorsWithDate(
        date
      );
  ...
  async updateProfilePhoto(mentor: Imentor, file: file): Promise<ResponseObj> {
    ...
    const imagePath = await this.staticFile.uploadFile(
      file.buffer,
      file.originalname
    );
  ...
  async storeRequest({ data, mentorRVId, user }): Promise<ResponseObj> {
    const resData = await this.reviewTimeRepository.storeRequest(
      data,
      mentorRVId,
      user._id
    );
